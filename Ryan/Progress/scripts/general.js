function isChinese() {
  let lan = $app.info.locale.substring(0, 2)
...
function shadow(view, isButton = false) {
  let corner = isButton ? 5 : 5
  let offset = isButton ? $size(0, 0) : $size(3, 3)
  let opacity = isButton ? 0.3 : 0.5
  let radius = isButton ? 3 : 5
  
  let layer = view.runtimeValue().$layer()
  layer.$setCornerRadius(corner)
  layer.$setShadowOffset(offset)
  layer.$setShadowColor($color("darkGray").runtimeValue().$CGColor())
...
function viberate(view, frame, stack = [0, 15, -15]) {
  if (stack.length > 0) {
    let val = stack.pop()
    $ui.animate({
      duration: val === 0 ? 0.5 : 0.05,
      damping: val === 0 ? 0.1 : 0,
      animation: function() {
        let newFame = $rect(frame.x, frame.y, frame.width, frame.height)
        newFame.y += val
        view.frame = newFame
      },
      completion: function() {
        viberate(view, frame, stack)
      }
    })
  }
}

function attributedString(text, colors) {
  let string = $objc("NSMutableAttributedString").$alloc().$initWithString(text)
  ...
  while (index != -1) {
    ...
    string.$addAttribute_value_range("NSColor", colors[count], $range(index, end-index))
    string.$addAttribute_value_range("NSFont", $font("bold", 13), $range(index, end-index))
